<x-app-layout :minimal-footer="true">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div>
                ...
                @if ($search !== '')
                    <p class="mt-1 text-sm text-gray-500">
                        ...
                        @endif
                        &mdash; <a href="{{ route('blog.index', $sort !== 'recommended' ? ['sort' => $sort] : []) }}" class="text-primary-600 hover:underline">{{ __('blog.view_all') }}</a>
                    </p>
                @elseif ($category)
                    <p class="mt-1 text-sm text-gray-500">
                        {{ __('blog.posts_in_category') }}:
                        <span class="font-medium text-gray-700">{{ $category->getName() }}</span>
                        — <a href="{{ route('blog.index', $sort !== 'recommended' ? ['sort' => $sort] : []) }}" class="text-primary-600 hover:underline">{{ __('blog.view_all') }}</a>
                    </p>
                @else
            ...
            {{-- Sort + Search --}}
            <form method="GET" action="{{ route('blog.index') }}" class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto sm:items-center">
                @if ($category)
                    <input type="hidden" name="category" value="{{ $category->slug }}">
                @endif
                <div class="flex items-center gap-2 shrink-0">
                    <label for="blog-sort" class="text-sm text-gray-500 whitespace-nowrap">{{ __('blog.sort_by') }}:</label>
                    <select id="blog-sort" name="sort" onchange="this.form.submit()"
                        class="rounded-lg border border-gray-200 px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-transparent">
                        <option value="recommended" {{ $sort === 'recommended' ? 'selected' : '' }}>{{ __('blog.sort_recommended') }}</option>
                        <option value="newest" {{ $sort === 'newest' ? 'selected' : '' }}>{{ __('blog.sort_newest') }}</option>
                        <option value="oldest" {{ $sort === 'oldest' ? 'selected' : '' }}>{{ __('blog.sort_oldest') }}</option>
                        <option value="most_commented" {{ $sort === 'most_commented' ? 'selected' : '' }}>{{ __('blog.sort_most_commented') }}</option>
                    </select>
                </div>
                <div class="relative flex-1 sm:w-72 shrink-0">
                        ...
                        @foreach ($posts as $post)
                            <article class="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-gray-200 transition-all overflow-hidden flex flex-col">
                                ...
                                @else
                                    ...
                                    <a href="{{ route('blog.show', $post->slug) }}" class="block aspect-video flex items-center justify-center p-4 text-center" style="background-color: {{ $ph['bg'] }}; color: {{ $ph['text'] }};">
                                        <span class="text-sm font-semibold line-clamp-3">{{ $post->getTitle() }}</span>
                                    </a>
                                @endif
            ...
            <aside class="hidden lg:block lg:col-span-1 space-y-6 mt-8 lg:mt-0">
                ...
                @if ($categories->isNotEmpty())
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                        ...
                        <ul class="space-y-1">
                            <li>
                                <a href="{{ route('blog.index', $sort !== 'recommended' ? ['sort' => $sort] : []) }}"
                            ...
                            @foreach ($categories as $cat)
                                <li>
                                    <a href="{{ route('blog.index', array_filter(['category' => $cat->slug, 'sort' => $sort !== 'recommended' ? $sort : null])) }}"
