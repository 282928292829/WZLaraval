<nav x-data="{ open: false, infoOpen: false }"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="relative flex items-center h-14">
            ...
            {{-- Right: cart icon (when cart layout enabled, hidden on new-order page) + auth controls + hamburger --}}
            <div class="flex items-center gap-1.5 ms-auto shrink-0 z-10">

                @php
                    $showNavCart = (\App\Models\Setting::get('order_new_layout', '1') === '2') && ! request()->routeIs('new-order');
                @endphp
                @if ($showNavCart)
                    <a href="{{ url('/new-order') }}"
                       class="flex items-center justify-center w-10 h-10 rounded-lg text-gray-600 hover:text-primary-600 hover:bg-primary-50 transition-colors"
            ...
            @auth
                @can('view-all-orders')
                    <div class="border-t border-gray-100 pt-2 mt-2">
                        <p class="px-3 py-1.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('nav.team') }}</p>
                        <a href="{{ route('orders.all') }}" @click="open = false"
                           class="flex items-center px-3 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
                            {{ __('staff.all_orders') }}
                        </a>
                        <a href="{{ route('comments.index') }}" @click="open = false"
                           class="flex items-center px-3 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-lg transition-colors">
                            {{ __('Comments') }}
                        </a>
