<x-app-layout>
    ...
    <section class="relative overflow-hidden bg-gradient-to-br from-orange-50 via-white to-amber-50">
        ...
        <div class="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-20 sm:pt-24 sm:pb-28 text-center">
            ...
            <span class="inline-flex items-center gap-1.5 rounded-full bg-orange-100 text-orange-700 text-xs font-semibold px-3 py-1 mb-6 tracking-wide uppercase">
                ...
                @if(app()->getLocale() === 'ar') خدمة موثوقة @else Trusted Service @endif
            </span>

            <h1 class="text-4xl sm:text-5xl lg:text-6xl font-bold text-gray-900 leading-tight mb-5">
                @if(app()->getLocale() === 'ar')
                    اشترِ من <span class="text-orange-500">أي متجر</span> حول العالم
                @else
                    Shop from <span class="text-orange-500">any store</span> worldwide
                @endif
            </h1>

            <p class="text-lg sm:text-xl text-gray-500 max-w-2xl mx-auto mb-8 leading-relaxed">
                @if(app()->getLocale() === 'ar')
                    أرسل لنا روابط المنتجات التي ترغب بشرائها، واترك الشراء والتغليف والشحن لباب بيتك علينا. نشتري من أمازون وجميع المواقع العالمية.
                @else
                    Send us the product links you want to buy. We handle the purchase, packaging, and shipping straight to your door — from Amazon and all global stores.
                @endif
            </p>
            ...
            <form id="hero-order-form" action="{{ route('new-order') }}" method="GET" class="w-full max-w-2xl mx-auto">
                <div class="flex flex-col sm:flex-row gap-2 bg-white rounded-2xl shadow-lg shadow-gray-200/60 border border-gray-100 p-2">
                    ...
                    <button type="submit"
                        ...
                        @if(app()->getLocale() === 'ar') ابدأ الطلب @else Start Order @endif
                    </button>
                </div>
            </form>
            ...
            <div class="mt-5">
                <a href="https://wa.me/00966556063500"
                   ...
                   class="inline-flex items-center gap-2 text-gray-500 hover:text-green-600 text-sm font-medium transition-colors duration-150">
                    ...
                    @if(app()->getLocale() === 'ar') أو اطلب على الواتساب @else Or order on WhatsApp @endif
                </a>
            </div>
        </div>
    </section>
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                ...
                <p class="text-gray-500 max-w-xl mx-auto">
                    @if(app()->getLocale() === 'ar')
                        ثلاث خطوات بسيطة وطلبك في طريقه إليك
                    @else
                        Three simple steps and your order is on its way
                    @endif
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                @foreach([
                    [
                        ...
                        'title_ar' => 'أرسل رابط المنتج',
                        'title_en' => 'Send the product link',
                        'desc_ar' => 'انسخ رابط المنتج من أي متجر وأضفه إلى نموذج الطلب.',
                        'desc_en' => 'Copy the product URL from any store and paste it into the order form.',
                    ],
                    [
                        'step' => '02',
                        'icon' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2',
                        'title_ar' => 'نتولى الشراء',
                        'title_en' => 'We handle the purchase',
                        'desc_ar' => 'فريقنا يراجع طلبك ويشتري المنتجات نيابةً عنك.',
                        'desc_en' => 'Our team reviews your order and purchases the items on your behalf.',
                    ],
                    [
                        'step' => '03',
                        'icon' => 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4',
                        'title_ar' => 'نشحن إليك',
                        'title_en' => 'We ship to you',
                        'desc_ar' => 'نشحن طلبك مباشرةً إلى عنوانك مع متابعة كاملة.',
                        'desc_en' => 'We ship your order directly to your address with full tracking.',
                    ],
                ] as $item)
    ...
    <section class="py-16 sm:py-20 bg-gray-50 border-t border-gray-100">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                <h2 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-3">
                    @if(app()->getLocale() === 'ar') لماذا وسيط زون؟ @else Why Wasetzon? @endif
                </h2>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                @foreach([
                    [
                        'icon' => '📦',
                        'title_ar' => 'إزالة التغليف الزائد',
                        'title_en' => 'Remove excess packaging',
                        'desc_ar' => 'نقلل حجم الطرد عبر إزالة التغليف الزائد لخفض تكاليف الشحن.',
                        'desc_en' => 'We reduce package size by removing excess packaging to lower your shipping costs.',
                    ],
                    [
                        'icon' => '🏠',
                        'title_ar' => 'مقرنا في ديلاوير — بدون ضريبة',
                        'title_en' => 'Based in Delaware — tax free',
                        'desc_ar' => 'مستودعنا في ولاية ديلاوير معفى تماماً من ضرائب المبيعات الأمريكية.',
                        'desc_en' => 'Our warehouse in Delaware is fully exempt from US sales tax.',
                    ],
                    [
                        'icon' => '💰',
                        'title_ar' => 'وفّر حتى 70%',
                        'title_en' => 'Save up to 70%',
                        'desc_ar' => 'نجمع طلباتك من متاجر مختلفة في طرد واحد ونوفر عليك مبالغ طائلة.',
                        'desc_en' => 'We consolidate your orders from different stores into one package and save you a fortune.',
                    ],
                    [
                        'icon' => '⏲️',
                        'title_ar' => '90 يوم تخزين مجاني',
                        'title_en' => '90 days free storage',
                        'desc_ar' => 'نمنحك حرية التسوق على مدار 3 أشهر مع تخزين مجاني وآمن لمنتجاتك.',
                        'desc_en' => 'We give you the freedom to shop for 3 months with free and secure storage for your products.',
                    ],
                ] as $feat)
                <div class="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm flex items-start gap-4">
                    ...
                    <div>
                        <h3 class="font-semibold text-gray-800 mb-1">
                            {{ app()->getLocale() === 'ar' ? $feat['title_ar'] : $feat['title_en'] }}
                        </h3>
                        <p class="text-sm text-gray-500 leading-relaxed">
                            {{ app()->getLocale() === 'ar' ? $feat['desc_ar'] : $feat['desc_en'] }}
                        </p>
                    </div>
                </div>
                @endforeach
    ...
    <section class="py-16 sm:py-20 bg-orange-500">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 class="text-2xl sm:text-3xl font-bold text-white mb-4">
                @if(app()->getLocale() === 'ar')
                    جاهز للبدء؟ قدّم طلبك الآن
                @else
                    Ready to start? Place your order now
                @endif
            </h2>
            <p class="text-orange-100 mb-8 text-base">
                @if(app()->getLocale() === 'ar')
                    سجّل حساباً مجانياً وقدّم طلبك الأول في دقائق.
                @else
