{{--
  Layout: Table Sticky (Option C)
  Viewport-fill: height calc(100dvh - 56px) — adapts to mobile browser chrome.
  Single overflow-auto scroll container handles BOTH x and y axes.
  Sticky thead works because parent is overflow-auto (not overflow-x-auto).
  Footer is inline at bottom of flex column — NOT position:fixed.
  Test URL: /new-order-table-sticky
--}}
...
<div
    ...
    x-data="newOrderFormTableSticky(
        @js($exchangeRates),
        @js($currencies),
        {{ $maxProducts }},
        @js($defaultCurrency),
                        ...
                        <tr class="border-b border-slate-100 hover:bg-slate-50/60 transition-colors group align-top">
                            ...
                            <td class="py-2 px-2">
                                <div class="flex items-start gap-1 flex-wrap min-h-[2rem]">
                                    <template x-for="(f, fi) in (item._files || [])" :key="'tbl-sk-file-'+idx+'-'+fi">
...
{{-- shrink-0 stays at bottom of flex column; no position:fixed needed --}}
...
<script>
function newOrderFormTableSticky(rates, currencyList, maxProducts, defaultCurrency, isLoggedIn, commissionSettings, initialItems, initialOrderNotes, maxImagesPerItem, maxImagesPerOrder, msgMaxPerItem, msgMaxOrder, testOptions, allowedMimeTypes, maxFileSizeBytes) {
    return {
        ...
        // Fix: pushed items include _id for stable x-for keying
        addFiveTestItems() {
            const urls = [
