...
<x-app-layout :minimal-footer="true">
    <div class="max-w-5xl mx-auto px-4 py-4 sm:py-6 space-y-3">
        ...
        <div x-data="{ open: {{ request()->hasAny(['search','status','sort','per_page']) ? 'true' : 'false' }} }">
            ...
            <form method="GET" action="{{ $listRoute }}"
                  x-show="open" x-cloak
                ...
                <div class="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1.5">{{ __('Search') }}</label>
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Order number...') }}"
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1.5">{{ __('Status') }}</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1.5">{{ __('Sort') }}</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1.5">{{ __('Per page') }}</label>
        ...
        @if ($orders->isEmpty())
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm text-center py-16 px-6">
                ...
                <p class="mt-1.5 text-sm text-gray-400 max-w-xs mx-auto leading-relaxed">
                    @if (request()->hasAny(['search','status'])) {{ __('No orders match your filters. Try different criteria.') }}
                    @else {{ __('Your orders will appear here once you place one.') }} @endif
                </p>
        ...
        @else
            ...
            {{-- Simple list: rows with dividers, no borders or shadows --}}
            <div class="md:hidden bg-white divide-y divide-gray-200 overflow-hidden">
                @foreach ($orders as $order)
                    ...
                    @endphp
                    <a href="{{ route('orders.show', $order->id) }}" class="block px-4 py-3 hover:bg-gray-50 transition-colors">
                        <div class="grid grid-cols-3 gap-2 items-center">
                            ...
                            <div class="min-w-0">
                                ...
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ring-1 ring-inset {{ $statusClasses }}">{{ $order->statusLabel() }}</span>
                            </div>
                        </div>
                        <div class="mt-2">
            ...
            <div class="hidden md:block bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                            <th class="px-4 py-3 text-start text-xs font-semibold text-gray-600 uppercase tracking-wide">{{ __('Order #') }}</th>
                            <th class="px-4 py-3 text-start text-xs font-semibold text-gray-600 uppercase tracking-wide">{{ __('Date') }}</th>
                            <th class="px-4 py-3 text-start text-xs font-semibold text-gray-600 uppercase tracking-wide">{{ __('Status') }}</th>
                            <th class="px-4 py-3 text-start text-xs font-semibold text-gray-600 uppercase tracking-wide w-24">{{ __('Actions') }}</th>
                        </tr>
                    </thead>
                        ...
                        @foreach ($orders as $order)
                            ...
                            <tr class="hover:bg-gray-50/60 transition-colors cursor-pointer" onclick="location.href='{{ route('orders.show', $order->id) }}'"
                                ...
                                <td class="px-4 py-3" onclick="event.stopPropagation()">
                                    <a href="{{ route('orders.show', $order->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-primary-600 bg-primary-50 hover:bg-primary-100 border border-primary-200 rounded-lg transition-colors">{{ __('Open') }}</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
